...
该仓库是我参加 [AI Web3 School BootCamp](https://aiweb3.school/) 的顺手打造的 Agent 驱动管理的 "工作台"——从开营, 共学营, 后续的 Hackathon 项目全链路呈现的仓库笔记，包括不限于: handbook, personal note、Co-Learning topic、每日打卡、踩过的坑、最后那个能跑起来（希望能跑起来并且长期稳定可维护）的项目，都作为资产积累沉淀在此。

## 起源 orgin

 🎯 目标:

> 学习 新兴的 web3 范式 和 AI 生态的结合驱动,  Agent 在全链路中通过 function call、读写数据、触发支付的更加高效的推荐web3的自动化, 高效化

授权、签名、不可伪造的执行记录、可撤销的能力——这些恰好是 Web3 这十年攒下来的基础设施在做的事。而 AI 

但两边的人现在还各说各话：

​	AI 圈子里很多人不知道 Session Key 是啥

​	Web3 圈子里很多人觉得 Agent 只是"接个 API 而已"。

我想花这 4 周投入在这个交叉地带，把两套语言学到能互译，最后做出一个能在测试网上跑起来的MVP, 当然后续有机会, 可以继续完善迭代, 这个仓库就是这个过程的复现
