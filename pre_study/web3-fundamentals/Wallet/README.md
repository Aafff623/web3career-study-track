...
> *知识节点：钱包是意图的最后一道门* 钱包不是登录按钮，而是"用户意图→链上执行"之间的确认界面。连接 ≠ 授权，签名 ≠ 登录，这三层权限的混淆是新手最常犯的错误。
...
> *知识节点：钱包 = 控制权管理器* 钱包的本质是管理链上控制权的工具，不是"账号系统"。平台可以封你的微博号，但不能转走你的 ETH——这就是控制权归属的根本差异。
...
> *知识节点：EOA = 私钥直接控制* EOA 是最简单也最脆弱的账户模型：一把私钥控制一切。简单通用，但丢失难恢复、权限难细分、自动化能力弱。这是 Account Abstraction 要解决的问题。
...
> *知识节点：助记词 = 私钥的备份形式* 助记词能还原所有私钥，等同于控制权本身。任何索要助记词的行为都是钓鱼，没有例外。正确做法是在钱包应用内部恢复，不交给任何第三方。
...
> *知识节点：交易 = 不可逆的状态变更* 交易一旦上链就不能随便撤回，这是和 Web2 操作的根本区别。Agent 辅助交易时，必须让用户在签名前完全理解交易内容。
...
> *知识节点：Gas = 执行成本的透明化* Gas 不是"手续费"这么简单，它直接影响交易能否被打包、要等多久、失败了是否还扣钱。产品必须把这些信息解释清楚，不能只显示一个数字。
...
> *知识节点：Explorer = 链上事实的只读窗口* Explorer 不是链本身，而是链的"浏览器"。Agent 引用链上信息时应该给出 explorer 链接，让用户自己验证，而不是只相信模型总结。
...
> *知识节点：Agent-钱包分工* AI 做理解和辅助（解释交易、检查风险、准备参数），钱包负责确认和授权（签名、权限控制），策略合约或智能账户负责执行约束。三层分工缺一不可。
