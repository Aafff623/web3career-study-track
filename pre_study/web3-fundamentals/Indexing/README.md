...
> *链上是事实来源，索引层是可用数据层。AI Agent 的推理质量直接取决于索引层提供的事实质量——垃圾进，垃圾出。*
...
> *Event 是 AI Agent 获取链上上下文的最自然入口——结构化、可过滤、带参数，比 raw calldata 友好得多。*
...
> *Subgraph 让 AI Agent 不必自己写 indexer，就能通过 GraphQL 获取结构化链上数据，是 Agent 上下文层的基础设施。*
...
- 列出它应该发出的 event。
- 设计一张查询表，例如 votes、transfers 或 mints。
- 标出每个字段来自哪个 event 参数或交易字段。
