...
> *Web3 安全的核心不是"没有 bug"，而是把可预见风险尽量挡在执行前，并在执行后能快速发现和止损。AI Agent 的引入让攻击面从合约层扩展到了模型推理层。*
...
> *Reentrancy 的本质是"状态更新滞后于外部调用"。Checks-Effects-Interactions 模式就是为此设计的：先检查、再改状态、最后才交互。*
...
- Solidity Security Considerations：官方安全注意事项，包含 reentrancy、gas、随机数和合约交互风险。
...
> *权限设计是 AI Agent 安全的核心前提——Agent 能调用哪些工具、能转多少钱、能改哪些参数，都必须在权限层预先约束。*
...
> *审计报告是一份特定时间点的快照。上线后的每次 upgrade、参数变更、新增 tool 都可能重新引入风险，安全是持续过程而非一次性事件。*
...
> *对 AI Agent 而言，Simulation 是"模型建议"和"链上执行"之间的安全闸门——先模拟再执行，能挡住绝大多数由幻觉或错误推理导致的危险交易。*

Simulation 不能替代安全审计，因为真实链状态可能在交易打包前变化。但它能挡住很多明显错误：链 ID 错、合约地址错、授权额度异常、滑点过大、余额不足、调用方法不符合预期。

相关 topic

- Tenderly Simulations：了解交易模拟、资产变化和执行 trace 如何用于调试和风控。
...
> *监控本身还不够。真正有效的是"监控 + 响应"：谁收到告警，谁能 pause，谁确认误报，恢复流程是什么。这对 Agent 系统尤为重要——Agent 需要被监控，也需要有熔断机制。*
...
- 写出如果这笔交易由 Agent 发起，执行前要做哪些 simulation 和 human check。
- 写出上线后应该监控哪些 event 或异常指标。


## 扩展阅读

- Solidity Security Considerations：官方安全注意事项，适合建立合约安全基础。
- OpenZeppelin Contracts：学习常用安全组件、访问控制和标准合约实现。
- OpenZeppelin Utils：查看 ReentrancyGuard、Pausable、Nonces 等工具。
- Ethereum Smart Contract Security：从以太坊开发者文档理解常见安全实践。
- Tenderly Simulations：学习交易模拟和执行 trace 如何帮助排查风险。
