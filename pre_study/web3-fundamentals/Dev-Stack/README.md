...
> *六步链路是 Web3 开发的"最小闭环"：写 -> 编译 -> 部署 -> 测试 -> 前端 -> 验证。缺任何一步都会在后面制造排查成本。*
...
> *第一性原理：链上不可逆，所以一切错误必须前移。本地能跑、测试能复现、地址可追踪——三条铁律。*
...
3. 用 Deploy & Run 面板理解 read 和 write 调用的区别。

Remix 不适合长期替代工程化 repo。只要项目开始多人协作，就应该把合约放进 Git 仓库，并用 Hardhat 或 Foundry 固化测试和部署流程。

> *Remix 是合约实验台——快速验证语法和调用流程，但不是工程化方案。多人协作开始时，必须迁移到 Hardhat/Foundry + Git。*
...
> *Hardhat 是 JS/TS 生态的合约工程框架。记住典型 repo 结构（contracts/test/ignition/config/artifacts），这是理解任何 Hardhat 项目的起点。*
...
> *Foundry = Solidity-native 的重型武器。forge/cast/anvil 三件套覆盖测试-交互-本地链。fuzz testing 是安全审计的前置防线。*
...
> *OpenZeppelin 解决的是"轮子安全"，不解决"设计安全"。用库减少了基础 bug，但权限组合、升级策略、经济模型仍然是你的责任。*
...
2. 用户当前在哪条链。
3. 合约读取结果是否正在加载或已过期。
4. 写交易处于等待签名、已广播、等待确认、成功或失败的哪一步。

如果这些状态混在一个按钮里，用户会很难判断自己是在"连接钱包""签名授权"还是"等待交易上链"。

> *前端接链的四大状态（连接 / 网络 / 读取 / 写交易）必须分离管理。状态混在一起 = 用户困惑 + 难以 debug。viem/wagmi 降低复杂度，但状态设计是你的责任。*
...
> *工具链串联示例：Remix(原型) -> Hardhat/Foundry(工程化) -> 本地链(部署测试) -> viem/wagmi(前端) -> 区块浏览器(验证)。看起来慢，但省掉的是上线后的排查成本。*
...
> *AI Agent 介入开发栈后，验证门槛反而要提高：Agent 能跑 forge test / cast / 部署脚本，但高风险命令（部署、转账、权限变更）必须人工确认。AI 提速，人类兜底。*
...
- 用 Remix 部署一个极简计数器合约，包含 count()、increment() 和 CountChanged event。
- 用 Hardhat 或 Foundry 建一个本地工程，并写测试覆盖初始值和 increment() 后的状态变化。
- 记录合约地址、ABI、部署网络、部署账号和交易哈希。
- 用 viem 或 wagmi 从前端读取 count()，再发起一次 increment() 交易。
- 在区块浏览器或本地日志里确认 event 被正确发出。
- 写下这条链路中哪些信息必须进版本控制，哪些必须放在 .env 或密钥管理里。
