...
> *合约是 Web3 的"不可逆层"——部署即生效，修改即风险。前端可以迭代，合约一锤定音。*
...
**知识节点：合约即状态机**——理解"状态 + 调用 + 事件"三位一体是读懂一切合约的基础。
...
> *链上代码是透明的——你的 bug 是所有人的 attack surface。三原则记住：状态裸奔、调用付费、权限显式。*
...
> *Solidity 的核心不是语法糖，而是 storage/msg.sender/event/revert 这几个链上特有概念。读和写是两种完全不同的操作——读免费，写付 gas 且需签名。*

相关 topic
- Solidity Documentation：官方语言文档，适合系统学习合约结构、类型、函数、事件、错误处理和安全注意事项。
...
> *EVM 是智能合约的"操作系统"。gas 计费、storage 热存储、跨链兼容性——所有链上"反直觉"行为的根源都在 EVM 的执行模型里。*

相关 topic
- 网络（Network）：继续看交易如何被网络打包、共识和确认。
...
> *ABI 对 AI Agent 就像 API doc 对人类开发者——告诉你能调什么，但不告诉你该不该调。Agent 拿到 ABI 后还需要业务上下文（decimals、权限、allowance）才能安全编码。*
...
> *Event 是链上世界的"webhook"——合约主动广播，外部系统被动监听。前端产品体验（历史记录、看板）依赖 event 索引，不是依赖直接读 storage。没有 event = 没有可查询历史。*

相关 topic
- 索引（Indexing）：继续看事件如何进入 subgraph、数据库和产品查询层。
...
判断升级风险时，可以直接问四个问题：
1. 升级权限在谁手里：单个 EOA、多签、DAO，还是 timelock 合约？
2. 用户是否能提前看到升级提案和新实现地址？
3. 升级能不能改变资产转移、提款、暂停或权限逻辑？
4. 如果管理员密钥泄漏，最坏结果是什么？

如果一个协议说"合约已审计"，但没有说清楚升级权限，这个安全说明仍然是不完整的。

> *升级 = 管理员权限的终极形式。四个问题清单（谁有权、能否提前看到、能改什么、密钥泄漏后果）是评估任何可升级协议的安全基线。审计不提升级权限 = 不完整审计。*
...
> *一次"点击按钮"背后的完整链路：钱包 -> RPC -> EVM -> Event -> Indexer -> 前端。合约只是其中一环，理解全局才能 debug 和设计好系统。*
...
> *AI x Web3 的安全架构：AI 建议 -> 钱包授权 -> 合约执行 -> 监控记录。模型负责"聪明"，合约负责"确定"，钱包负责"确认"。职责边界清晰才安全。*
