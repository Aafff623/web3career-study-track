...
> *知识节点：密码学即产品边界* 密码学不是理论课，而是 Web3 产品的安全边界定义。私钥=资产控制权，签名=授权证据，哈希=数据承诺，Merkle Tree=批量验证结构。每一个都对应一个用户可能犯的致命错误。
...
> *知识节点：控制权的数学化* 传统账户靠"用户名+密码+客服找回"，Web3 靠"私钥→公钥→地址"的单向数学关系。没有客服，没有重置，这就是为什么私钥管理是 Web3 的第一课。
...
> *知识节点：哈希 ≠ 加密* 哈希是单向的——能算出指纹，不能从指纹还原原文。这和加密（可逆）完全不同。链上用哈希做"数据指纹"而非"数据保护"。
...
> *知识节点：地址 ≠ 身份* 地址只是"能控制某把私钥"的证明标识，不是平台分配的身份。某个地址"看起来像官方地址"没有意义，真正重要的是它是否来自可信来源。
...
> *知识节点：私钥的不可恢复性* 传统密码丢了找客服，私钥丢了就是真丢了。泄漏 = 资产裸奔。这是 Web3 和 Web2 最根本的安全差异。
...
> *知识节点：签名 = 授权证据* 签名不是"确认登录"，而是对具体内容的授权。Agent 场景下更关键——模型不能代替用户签名，签名必须有明确的权限边界（金额、时间、合约）。
...
> *知识节点：Merkle Tree = 批量验证的效率工具* 核心价值：用一个 root hash 代表整批数据，用一个 proof 验证单条数据。空投名单、轻客户端、状态证明全靠这个结构。
...
> *知识节点：Agent 的密码学边界* Agent 能做的：解释签名内容、判断风险等级、提醒用户注意授权细节。Agent 绝不能做的：保管私钥、代替用户签名、在用户不理解时推动确认。这是硬红线。
