...
> *批注：DeFi 的本质是金融规则的协议化。AI Agent 要介入 DeFi，第一步不是执行交易，而是理解交易背后的资产状态、流动性和风险传导链。*
...
> *批注：DeFi 的三条铁律——资产即输入、流动性即执行能力、可组合性即风险放大器。这决定了 AI Agent 做 DeFi 决策时必须同时校验多层状态，不能只看单一价格。*
...
**知识节点：Token 是 DeFi 的资产原语，decimals 和权限模型直接影响协议行为，Agent 工具若不校验地址和精度就会产生资金风险。**
...
**知识节点：AMM 用 x*y=k 曲线把定价权交给池子状态，滑点和无常损失是流动性提供者的核心代价，MEV 则是交易排序层面的系统性风险。**
...
**知识节点：借贷协议的风险是多因素耦合的——抵押品波动、预言机延迟、清算流动性三者中任意一个出问题，都可能触发连锁坏账。AI 建议必须包含风险参数解释。**
...
**知识节点：稳定币的"稳定"取决于其底层机制（法币储备/超额抵押/算法），每种机制有各自的脱锚风险，Agent 需要识别稳定币类型才能评估 DeFi 组合中的系统性风险。**
...
**知识节点：流动性是 DeFi 的"执行层基础设施"，价格只是流动性的投影。Agent 做任何 DeFi 操作前，必须评估目标资产在目标池子中的深度。**
