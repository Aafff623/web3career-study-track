...
> *知识节点：钱包与权限 = 执行型 Agent 的地基* 错误判断可能变成真实链上操作，不能像 Web 应用那样撤回。决定：Agent 能读还是能写 / 用户能否看懂授权 / 权限是否有边界 / 出错能否撤销。
...
> *知识节点：Agent 不拥有钱包，只拥有可限制的能力* 把私钥给 Agent = 最脆弱设计。Agent 只提出意图 → 生成候选交易 → 请求有限权限 → 规则允许时触发。权限比意图更具体，签名前要可理解，撤销是默认能力。
...
> *知识节点：AI Wallet UX = 让确认变得有意义* 不是把聊天框接到钱包。至少展示：目标/行动计划/权限请求/交易解释/风险提示/撤销入口。重点是让用户知道 Agent 在做什么。
...
> *知识节点：Permission Policy = 写给执行层的规则* 能被程序检查，不是只靠模型自觉。维度：资产范围/金额上限/目标合约/函数范围/滑点阈值/时间窗口/频率限制。最终由钱包/guard/合约校验。
...
> *知识节点：Session Key = 临时受限 key，不能绕过风险控制* 减少重复签名，但额度/时间/函数必须受限。太大太长太宽 = 把主钱包风险换了个名字。
...
> *知识节点：Guard = 确定性规则拦截 + 人工确认灰区* 不关心模型为什么建议，只关心是否符合规则。检查：地址白名单/函数白名单/额度/滑点/接收地址/模拟结果/频率。
...
> *知识节点：ERC-4337 = 权限和逻辑进入账户层* Agent 生成意图 → 包装成 UserOperation → Smart account 校验 → Bundler 打包 → Paymaster 代付 gas → 链上执行。权限/gas/批量执行/恢复逻辑在账户层，不在模型提示词。
...
> *知识节点：签名前模拟 = 必须做的基础动作* 模型解释只是辅助，安全判断看模拟结果。回答：余额变化/授权/合约调用/是否 revert/滑点/额外风险。关键字段来自结构化解析 + 链上模拟。
...
> *知识节点：恢复/撤销 = 给权限就必须设计* 用户要能在一个地方看到：所有 Agent/session key / 权限范围 / 已用额度 / 过期时间 / 最近动作 / 暂停撤销入口。高价值场景需要 guardian + 多签 + 延迟执行 + 社交恢复。
...
> *知识节点：按风险逐步开放四阶段* 只读 → 交易草稿+人工签名 → 小额白名单短期 session key → 复杂自动化+guard+审计+撤销。
...
1. 允许操作的 token、目标 DEX、最大单笔金额和每日上限。
2. 允许调用哪些函数，禁止哪些函数。
3. session key 的有效期和撤销方式。
4. 交易模拟需要展示哪些字段。
5. 哪些情况必须重新请求用户确认。
6. 执行日志如何让用户复查。
...
- ERC-4337 Documentation：理解账户抽象的标准工作流和开发组件。
- EIP-4337：阅读 UserOperation、EntryPoint、bundler 和 paymaster 的规范原文。
- EIP-7579：了解模块化智能账户如何支持插件化权限。
- Safe Docs：学习多签、模块和 guard 在真实钱包系统里的设计。
- OpenZeppelin Defender：了解部署、监控、自动化和安全运维工具。
