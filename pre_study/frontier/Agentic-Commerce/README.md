...
> *知识节点：Agentic Commerce 难点在交易边界* 不是"AI 自动买东西"，而是：Agent 能否判断价值 / 预算如何机器可执行 / 服务方如何证明完成 / 不满意怎么退款 / 小额高频跨平台怎么做。
...
> *知识节点：商业自动化 = 结构化协议* 购买前 intent（买什么/预算/接受什么结果）→ 购买中约束（报价/有效期/退款条件）→ 购买后证据（结果/收据/日志/验收/争议路径）。Agent 不能替用户承担无限责任。
...
> *知识节点：Agent 购买 API 的核心不是自动付款* 是服务发现 + 价格透明 + 权限限制 + 调用日志。Agent 动态发现服务并完成调用，不是提前注册账号复制 key。
...
> *知识节点：Payment Intent = 用户意愿 → 机器可检查的边界* 包含目标/预算/资产/服务方白名单/质量要求/退款条件/有效期。Agent 在边界内决策，不能把边界解释得越来越宽。
...
> *知识节点：预算分层 = 安全阀* 任务预算 / 服务预算 / 时间预算 / 风险预算 / 失败预算。没有预算控制，一次循环错误就可能不断购买。
...
> *知识节点：任务完成证明 = 一组证据，不是单一证明* API 用请求 ID + 响应状态，数据用哈希 + 版本，推理用模型版本 + 输出哈希，执行用 tx hash + 事件日志。低价值用日志，高价值用签名/链上事件/托管仲裁。
...
> *知识节点：链上收据 = 跨平台对账 + 行为追踪* 保存足够事后核对的证据（payer/provider/金额/tx hash/输出 hash/验收状态），隐私数据只存哈希或加密引用。
...
> *知识节点：Escrow = 托管状态机降低不信任* 创建任务 → 资金托管 → 交付结果 → 验收检查 → 放款/退款/争议。高价值或主观结果不应该只靠模型自动放款。
...
> *知识节点：三个切入方向* ① 帮 Agent 购买服务 ② 帮服务方接受 Agent 支付并自动交付 ③ 帮用户管理预算/权限/消费审计。
