...
> *知识节点：真实环境风险来自 AI + Web3 两边* Demo 顺滑不代表安全。常见缺口：恶意提示词藏在文档里 / 不可信内容被当指令 / 工具权限过大 / secret 进入上下文 / 链上操作不可撤回。
...
> *知识节点：安全 Agent = 更小更清楚更可审计的行动空间* 上下文分层（系统规则/用户目标/外部资料/工具返回值）→ 工具最小授权（读/写/签名/转账/部署分级）→ 高风险动作可审计。
...
> *知识节点：威胁模型 = 先画清楚哪些路径危险* 至少问六个问题（能读什么/能调什么/连了什么账号/哪些会真实执行/用户确认什么/出错能否追踪）。风险分层：只读最低 → 草稿中等 → 签名授权最高。
...
> *知识节点：防御 Prompt Injection = 结构隔离，不是多写一句"不要听"* 外部资料只作 data / 检索结果带来源可信度 / 工具调用前检查目标和权限 / 高风险指令重新确认 / 输出结构化校验。
...
> *知识节点：工具权限五层分级* 公开只读 → 用户只读 → 草稿 → 写入 → 资产。越靠后越需要白名单/额度/模拟/人工确认/多签/延迟执行。
...
> *知识节点：恶意文档 Demo = 安全入门最佳练习* 在正常文档里藏越权指令，观察系统是否：标记为外部资料 / 拒绝越权 / secret 不进上下文 / 工具调用前检查权限 / 输出说明拒绝原因。
...
> *知识节点：Agent 请求受控动作，不拿秘密本身* "查询余额" ≠ 拿 RPC key；"生成交易草稿" ≠ 拿私钥。签名由钱包/smart account/多签完成，Agent 不保管主私钥。
...
> *知识节点：隐私保护 = 最小化 + 脱敏 + 本地优先* 只读必要数据 / 外部调用脱敏摘要 / 默认隐藏地址金额身份 / 用户可查看授权范围 / 敏感任务优先本地模型或 TEE。
...
> *知识节点：审计日志 = 事故后定位问题在哪一层* 不只看最终回答。记录：目标/读了什么/调了什么/每次输入输出错误/哪些触发确认/结果和失败原因。
...
> *知识节点：健康路线 = 四步递进* 只读解释 → 生成草稿 → 低风险工具 → 受限执行。每步都要权限 + 模拟 + 日志 + 人工确认。
...
1. 正常文档和恶意文档分别包含什么。
2. Agent 如何区分系统规则、用户目标和外部资料。
3. 哪些工具只读，哪些工具禁用，哪些工具需要确认。
4. 恶意指令触发时系统如何拒绝。
5. 日志如何记录读取、判断和拒绝过程。
