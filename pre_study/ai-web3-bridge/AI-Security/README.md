...
> *知识节点：安全核心 = 隔离 + 约束* 模型可以建议，但不能绕过权限；可以读上下文，但不能全信；可以调工具，但工具要有 policy 和日志。核心目标：不可信输入无法直接变成不受限执行。
...
> *知识节点：三层防线* ① 上下文 ≠ 指令（外部内容不能覆盖系统规则）② 工具权限隔离（读写分离、风险分级）③ 审计日志（能复盘模型读了什么、做了什么）。
...
> *知识节点：Prompt Injection 防护两步* ① 分层标注上下文可信级别（用户指令 > 系统规则 > 工具结果 > 外部内容）② 高风险工具不直接听模型——即使被诱导生成转账请求，wallet policy + human check 仍应拦住。
...
> *知识节点：Tool Abuse = 小动作累积* 不是一次大动作，而是高频调用、重复付款、参数偏离、访问 secret 等小动作累积。防护靠 tool permission + rate limit + budget + simulation + 独立异常检测。
...
> *知识节点：恶意上下文 ≠ 恶意指令* 除了显式攻击指令，还有错误事实（假地址 / 伪造审计 / 过期价格 / 污染 metadata）。链上事实优先从 RPC / explorer / verified source 读取，网页只作辅助。
...
> *知识节点：Key Safety 底线* secret 不进 prompt、不进模型输出、不进日志、不进 analytics。Agent 代表用户操作优先用 Smart Account policy / session key，不把 EOA 私钥放进自动化 runtime。
...
> *知识节点：权限隔离 = 能力分层 + 环境隔离* 只读查询 / 交易草稿 / 发送交易 / 撤销授权 / 大额支付是不同能力，不要给 Agent 万能工具。环境也要隔离：浏览器不碰钱包密钥，sandbox 不碰生产数据库。
...
> *知识节点：Sandbox 默认禁止高敏感资源* .env / ssh key / wallet seed / cookie / 生产 DB 凭证默认不可访问。浏览器 sandbox 特别重要——恶意 dApp 页面可能诱导签名，浏览器自动化不能和钱包签名无边界连接。
...
> *知识节点：审计日志要记完整链路* 不只记最终回答，要记：模型看到了什么上下文 → 为什么选某工具 → 工具返回什么 → policy 是否通过 → 用户是否确认。高价值系统把日志 hash 锚定到链上防篡改。
...
> *知识节点：Alert 必须连接响应动作* 只有告警没有响应 = 安全收益有限。发现异常后要有明确动作：暂停 Agent / 撤销 session key / 冻结 escrow / 通知用户 / 人工审核。低风险进队列，高风险立即打断。
...
> *知识节点：安全贯穿所有桥接层* Chain-aware Context 防恶意上下文，Web3 Tool Use 防工具滥用，Agent Wallet 防权限扩大，Governance AI 防信息操纵。目标不是模型永不犯错，而是犯错时无法直接造成不可接受损失。
...
- OWASP Top 10 for LLM Applications：理解 prompt injection、敏感信息泄漏和工具滥用等风险。
- OpenAI Safety Best Practices：学习 AI 应用安全设计原则。
- OpenAI Tools Guide：理解工具调用和结构化结果的安全边界。
- MetaMask Wallet Docs：理解钱包签名和交易请求为什么需要用户确认。
- Tenderly Simulations：用交易模拟降低错误执行风险。
