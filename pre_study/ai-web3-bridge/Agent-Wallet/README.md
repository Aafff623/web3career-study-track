...
> *知识节点：Agent Wallet 的核心定位* 不是"给 AI 一个钱包"，而是"给 Agent 一个可审计、可撤销、有边界的行动空间"。两个极端都不可取：完全不能执行 = 没用；权限太大 = 危险。中间那条路 = 有限授权 + 随时可撤。
...
> *知识节点：三条推论* ① Agent 不碰主私钥（主资产隔离）；② 钱包 = 权限表达系统（不是签名按钮）；③ 自动化必须可撤销。这三条是 Agent Wallet 设计的硬约束，违反任何一条都会产生系统性风险。
...
- Account Abstraction：先理解 AA 和普通钱包账户的区别。
- ERC-4337 Documentation：继续看 UserOperation、EntryPoint、Bundler、Paymaster。

> *知识节点：AA = 账户可编程* EOA 的权限模型是"有私钥就能做一切"；AA Wallet 把权限拆成可编程规则（谁能做、能做什么、什么时候失效）。这是 Agent Wallet 的技术基础——没有 AA，Agent 要么拿到全部权限，要么完全不能执行。
...
- Wallet：补上钱包、签名和账户的基础概念。
- Safe Modules：看智能账户如何通过模块支持自动化和自定义授权。

> *知识节点：Smart Account = 规则容器* 判断 Agent Wallet 靠谱与否的标准：它有没有把"允许自动操作"拆成具体的、可检查的规则（合约白名单、方法限制、额度上限、时间窗口）。模糊授权 = 不安全。
...
- Governance AI：理解 Agent 进入治理流程时，为什么更需要审批和记录。
- Safe Guards：看 Safe 如何在交易执行前后做规则检查。

> *知识节点：Agent 参与流程，不独占控制权* 在团队/DAO 场景，Agent 的角色是"提议者"而非"执行者"——它生成交易草稿，但最终执行需要多签确认。这是 Agent 安全的组织层面保障。
...
- Rhinestone Smart Sessions：看 session key 如何组合协议限制、额度和过期时间。

> *知识节点：Session Key ≠ 小号私钥* 私钥 = 全权限、无时间限制、无法自动撤销。Session Key = 受限权限 + 时间窗口 + 额度上限 + 随时可撤。这个区别决定了 Agent 能安全自动化的边界。
...
- AI Security：继续看 Prompt Injection、工具权限隔离和行为审计。

> *知识节点：Policy = 可执行的规则* 好的 policy 不是法律文本，而是系统能自动检查的条件（if-then 规则）。模糊的"低风险操作"无法被程序验证；"每天 ≤10 USDC + 白名单合约 + 滑点 ≤1%"可以。
...
- Safe Guards：看 Guard 在交易前后检查中的位置。

> *知识节点：Guard = 最后一道确定性防线* Agent 生成候选动作（概率性），Guard 拒绝越界动作（确定性）。这个分工是 Agent 安全的最后一道防线——即使模型被 Prompt Injection 攻击，Guard 也能用硬规则拦住。
...
- Tenderly Simulation API：看交易模拟、资产变化、gas 估算和 state override。

> *知识节点：Simulation = 用户理解的入口* 在 Agent 场景，simulation 不只是"技术上能不能发"，而是"用户看不看得懂自己在批准什么"。把模拟结果翻译成"你会花 X、得到 Y、授权 Z"是 human-in-the-loop 的核心体验。
...
> *知识节点：撤销 = 主动 + 被动* 撤销不只是用户手动关。系统也应在异常时自动收紧（到期、额度耗尽、多次失败、异常地址）。产品原则：所有自动化权限必须有一个可见的关闭按钮。
...
- Agent Workflow：理解哪些步骤适合自动执行，哪些步骤需要 human-in-the-loop。

> *知识节点：分层确认* 不是所有步骤都需要人确认（那样 Agent 没意义），也不是所有步骤都能自动（那样太危险）。分层：自动 → 模拟+确认 → 强制确认 → 直接拒绝。关键是让用户看懂"会改变什么"，而不是只看哈希。
...
> *知识节点：第 3-6 步是事故高发区* 大多数 Agent Wallet 的安全事故不是"模型不够聪明"，而是"从计划到执行之间缺了 Guard / Simulation / 确认 / 审计"。把这四步做好，比换更强的模型更重要。
...
- EIP-4337: Account Abstraction Using Alt Mempool：标准原文，适合看 UserOperation、EntryPoint、validation / execution 分离这些底层概念。
- ERC-4337 Documentation：比 EIP 更适合入门，解释了 Bundler、Paymaster、UserOperation 和安全检查流程。
- Safe Modules：理解 Safe 如何通过模块支持自动化、限额、白名单等扩展能力。
- Safe Guards：理解 Guard 如何在交易执行前后做检查，以及为什么 Guard 本身也需要审计和恢复机制。
- Rhinestone Smart Sessions：适合学习 session key 如何表达可组合的 onchain permissions，例如协议限制、额度和过期时间。
- Tenderly Simulation API：看交易模拟、资产变化、gas 估算和 state override，理解为什么 Agent 生成交易后还需要先模拟。
