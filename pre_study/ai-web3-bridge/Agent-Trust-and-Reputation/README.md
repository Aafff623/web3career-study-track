...
> *知识节点：信任 ≠ 分数* 一个黑盒"信任分"不够用。真正可靠的信任系统是一组可追溯、可比较、可解释的证据：做过什么、谁评价过、有没有抵押、失败了有没有代价。
...
> *知识节点：三要素* 可验证行为（不是自我声明）、绑定身份（没有稳定身份无法积累历史）、绑定任务（泛泛评价不如具体结果）。没有成本的承诺 = 空话。
...
> *知识节点：声誉要按任务类型拆 + 时间衰减* "什么都好"= 什么都不可信。声誉应按任务类型拆分（擅长 A ≠ 擅长 B），且要有时间衰减（两年前的好表现不代表今天）。
...
> *知识节点：Review 要绑定任务 + 防互刷* 泛泛五星 = 没用。好的 review：任务 ID + 交付 hash + 具体问题。评价者本身也需要身份，要能查到它和被评价 Agent 的历史交易关系。
...
> *知识节点：Attestation = 结构化可验证声明* 必须有：issuer + subject + claim + evidence + expiration + revocation。没有过期和撤销机制的 attestation = 可能过期的误导信息。
...
> *知识节点：Stake = 经济承诺，不是能力证明* 有 stake 说明"失败有代价"，不代表"能力一定强"。要和 validation + review + task history 一起看。设计时明确：抵押物 + 锁定期 + 释放条件 + 罚没条件。
...
> *知识节点：Slashing = 最后手段，不是默认操作* 必须先定义违约证据 + 挑战窗口 + 仲裁者 + 误罚处理。主观任务（质量判断）不要自动 slashing，进 dispute；明确可验证违约（超时 / 伪造 / 矛盾结果）才适合自动执行。
...
> *知识节点：能力验证 ≠ 任务结果验证* 能力验证说明"这个 Agent 大概率能做某类任务"；任务结果验证说明"这次交付是否合格"。两者要分开记录，不能用能力徽章代替单次验收。
...
- ERC-8004：了解 Trustless Agents 的 identity、reputation 和 validation registry。

> *知识节点：ERC-8004 = 公共承载层，不是中心化评分* 它提供身份 + 反馈 + 验证信号的标准化格式，不同应用可以在此基础上构建自己的排序规则。链上注册证明身份连续性，不能自动证明服务质量。
...
> *知识节点：信任是多维的* 不要只看一个分数。可靠的信任系统 = 身份 + 任务历史 + 评价者 + 证明 + stake + 争议记录 + 上下文适配度。单一维度容易被操纵。
