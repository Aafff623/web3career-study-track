...
> *知识节点：Escrow = 任务与资金的绑定层* 先付怕被骗，后付怕收不到。Escrow 把"资金锁定 → 任务交付 → 验收 → 释放"串成状态机，让双方都不需要盲目信任对方。
...
> *知识节点：先定义状态机，再写代码* Escrow 的核心不是"锁钱合约"，而是"任务在什么状态下、谁可以触发、需要什么证据、超时怎么办"。没有状态机的 escrow = 把双方困在合约里。
...
> *知识节点：最小状态机* Created → Funded → Delivered → Accepted → Released，异常分支：Refunded / Disputed。每个状态转换都需要明确：谁能触发、需要什么证据、超时怎么处理。
...
> *知识节点：Receipt = 人机双读* 收据要同时给人看（任务 + 金额）和给机器解析（task ID + tx hash + delivery hash + status）。它是对账、声誉和争议的共同数据源。
...
> *知识节点：Proof 必须与任务绑定* 交付证明不是"我说交付了"，而是"这个 hash / event / commit 对应那个任务"。没有绑定关系的 proof = 无法验证的口头承诺。
...
> *知识节点：AI 初审 + challenge window + 人工复核* 自动验收效率高但可能出错。三步组合：AI 先判 → 给 challenge 窗口 → 人工兜底。低风险自动过，高争议不被 AI 一次定生死。
...
> *知识节点：退款是协议的一部分* 不是失败后的"善意补救"，而是预先定义的规则。必须在任务开始前写清：触发条件、部分退款比例、申诉路径。
...
> *知识节点：Dispute = 有成本的正义* 无成本争议 → 滥用；成本过高 → 小额无法维权。设计要点：发起成本 + 证据格式 + 裁决权 + 申诉路径。争议记录也是声誉输入。
...
> *知识节点：Evaluator 本身也需要被评估* 错误率高的 evaluator = 随机判决机。必须记录版本、输入输出、错误率。组合使用（脚本 + 测试 + AI + 人类）比单一模型更可靠。
...
- ERC-8183: Agentic Commerce：了解 Agentic Commerce 的标准草案。

> *知识节点：ERC-8183 = 任务生命周期标准化* Agent 交易不是"转一笔钱"，而是"任务创建 → 状态推进 → 交付验证 → 资金释放"的完整生命周期。ERC-8183 试图把这个流程标准化。
...
> *知识节点：支付的后半段* Machine Payment 解决"怎么付"，Settlement & Escrow 解决"付了之后怎么确认价值交换完成"。没有后半段，支付就是单向转账，不是交易。
...
- ERC-8183: Agentic Commerce：Agentic Commerce 的任务、支付和结算标准草案。
- ERC-8004: Trustless Agents：理解 Agent 身份、声誉和验证如何辅助交易信任。
- AP2 Documentation：了解 Agent 支付授权和可审计证明。
- OpenZeppelin Contracts：学习 escrow、payment 和 access control 相关合约设计基础。
