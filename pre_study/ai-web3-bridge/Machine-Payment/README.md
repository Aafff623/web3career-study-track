...
> *知识节点：Intent ≠ Settlement* 机器支付的关键设计是把"想付钱"（Payment Intent）和"真的付了"（Settlement）拆开。Intent 是授权范围，Settlement 是实际执行。混在一起就无法审计"Agent 到底花了谁的钱、买了什么"。
...
> *知识节点：三原则* 预算先于执行（没预算就不能付款）、报价可比较（价格 / 币种 / 有效期 / 退款条件）、收据可验证（付给谁 / 为什么付 / 交付了什么）。缺任何一条，自动支付都会变成"黑洞消费"。
...
> *知识节点：稳定币 ≠ 只看面值* 同样是"0.1"，可能是 USDC / USDT / 原生 token，decimals 不同、链不同、流动性不同。Agent 必须校验 chain id + token address + decimals，不能只看数字。
...
> *知识节点：多层预算* 只设总预算 = 给 Agent 一张信用卡。正确做法：全局预算 → 任务预算 → 单次上限 → 服务方上限 → 频率限制 → 紧急停止。每层都是独立拦截点。
...
> *知识节点：Quote 必须有有效期和签名* 过期 quote = 过期承诺。没有签名的 quote = 无法追溯的口头协议。Agent 接受 quote 前必须检查：有效期、签名、收款地址绑定。
...
> *知识节点：Intent = 授权快照* Payment Intent 是用户授权的快照——"我同意为这个服务最多付 X"。它不是交易本身，而是后续所有支付行为的审计锚点。没有 Intent，Agent 的付款就变成了"自说自话"。
...
> *知识节点：x402 = HTTP 原生支付语义* 把"付费才能用"放回 HTTP 402 状态码。Agent 可以像处理 401 登录一样处理 402 付款——但这只解决"怎么付"，不解决"付了之后服务质量怎么保证"。
...
> *知识节点：MPP = 机器间的商务协议* 把"发现 → 报价 → 授权 → 结算 → 收据"协议化，让 Agent 不用临时拼流程。但不是所有步骤都要上链——链下协商 + 链上结算 / 收据锚定是更实际的模式。
...
> *知识节点：订阅 = 需要撤销的自动化* 订阅比单次付款更危险，因为它是持续扣款。必须有：可见的取消入口、月预算上限、扣款时间窗口、异常告警。无限 allowance 的订阅 = 定时炸弹。
...
> *知识节点：微支付 = 经济账先行* 高频小额场景，链上单笔结算的 gas 可能超过服务本身价值。正确做法：预存额度 → 累计消费 → 定期批量结算（L2 / payment channel / 链下账本 + 链上锚定）。
...
> *知识节点：支付不是孤立模块* Machine Payment 必须连接 Agent Wallet（权限边界）、Policy（规则检查）、Settlement（资金释放）、Receipt（凭证）。单独的"付款能力"只是半个功能。
