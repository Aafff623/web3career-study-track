...
> *知识节点：身份 = 可验证的连续性* Agent 不是一次性会话，而是跨平台、跨时间的经济参与者。身份的作用是让用户和其它 Agent 能验证"这个 Agent 还是昨天那个，它的能力和 owner 没有静默变更"。
...
> *知识节点：三要素* 可解析身份（从 id 找到 profile + endpoint）、可证明控制权（owner 签名更新）、可验证能力（测试 / 证明 / 历史记录）。三者缺一，身份就是空壳。
...
> *知识节点：Profile = 人机双读 + 更新历史* 好的 profile 同时服务人（描述 + 收费）和机器（endpoint + schemas + auth）。更新历史本身就是信任信号——静默变更 = 可疑行为。
...
> *知识节点：能力声明要具体 + 标风险* "我什么都能做" = 什么都不可信。好的 capability：输入 / 输出 schema + 价格 + 限制 + 风险等级（只读 / 草稿 / 自动执行）。越具体越有用。
...
> *知识节点：Endpoint = 身份的实际触点* 链上 id 只是锚点，endpoint 才是真正被调用的入口。劫持 endpoint = 劫持身份。更新必须 owner 签名 + 保留历史。
...
> *知识节点：Registry = 发现 + 连续性，不是万能信任* Registry 能证明"这个身份存在且连续"，不能证明"这个 Agent 好用或安全"。它是信任系统的第一层，不是全部。
...
- W3C DID Core：理解 Decentralized Identifiers 的基本模型。
- W3C Verifiable Credentials：理解可验证凭证如何表达声明和证明。

> *知识节点：DID/VC = 通用身份层* 不局限于某条链，可以承载能力证明、组织隶属、审计通过。但 VC 的可信度取决于 issuer——任何人都能签发 ≠ 任何声明都可信。
...
- Agent2Agent Protocol：了解 Agent 之间互操作和任务协作的开放协议。

> *知识节点：A2A = 通信层，不是身份层* 身份告诉你"在和谁说话"，A2A 负责"怎么协作"。支付场景中，A2A 消息必须和 Payment Intent / Receipt / Escrow 关联，否则对话和结算是两套系统。
...
> *知识节点：Operator ≠ Owner* 运行服务的人（operator）和控制身份的人（owner）可以分开。高价值 Agent 不应由单个热钱包控制——多签 / Smart Account / DAO 更安全。
...
> *知识节点：身份是信任的第一层* 先知道"是谁"，再看"做过什么"（reputation）"谁评价过"（review）"有没有抵押"（stake）"是否验证过"（validation）。身份 = 信任链的起点，不是终点。
...
- ERC-8004: Trustless Agents：了解 Agent 身份、声誉和验证 registry 的标准草案。
- ERC-8004 Website：快速理解 ERC-8004 的身份和 registry 模型。
- W3C DID Core：去中心化身份标准。
- W3C Verifiable Credentials：可验证声明的数据模型。
- Agent2Agent Protocol：Agent 间通信和互操作协议。
