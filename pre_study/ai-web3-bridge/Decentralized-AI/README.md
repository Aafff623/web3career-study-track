...
> *知识节点：去中心化 AI ≠ 把模型放链上* 是重新设计数据/模型/算力/推理/评测/收益分配的组织方式。核心问题：在哪些环节开放网络能提供更好的可验证性、可组合性和抗单点控制。
...
> *知识节点：分层判断价值* 数据/模型/算力/推理/评测/结算/治理是不同层，不能混成一个词。链上更适合做协调层（记录权属/激励贡献/结算任务），不直接承载大规模训练。
...
> *知识节点：模型市场 = 发现 + 评测 + 路由 + 结算* 不只是模型列表。关键是可替换性：便宜模型做分类/抽取，强模型做复杂推理，本地模型处理隐私，专用模型处理代码/图像/链上数据。
...
> *知识节点：数据市场不卖原始数据包* 围绕具体对象设计：可验证来源数据集 / 带授权条款数据 / 可复现 pipeline / 可审计贡献记录 / 标注样本。链上保存授权/哈希/版本/贡献/收益分配，不存大体量原始数据。
...
生产系统需要 SLA，而开放网络通常更难保证。所以 compute market 更适合先从容器化推理、批处理任务、非关键 workloads、可重复验证的任务做起。

> *知识节点：算力市场先从非关键任务做起* 难点：性能不稳定 / 可复现性差 / 结果需验证 / 隐私不可信。适合容器化推理、批处理、非关键 workloads。高价值任务需 TEE + ZK + 重复执行 + 抽样检查。
...
它和传统 API 的差别在于，推理节点不一定属于同一家公司。系统需要处理节点发现、模型版本、请求路由、速率限制、失败重试、结果验证和付款结算。

对开发者来说，最重要的是不要只看"能不能返回答案"，还要看：
...
> *知识节点：推理网络 = 节点发现 + 路由 + 验证 + 结算* 不只要看"能不能返回答案"，还要看：哪个节点 / 哪个版本 / 失败能否重试 / 隐私是否暴露 / 成本延迟质量是否记录。
...
> *知识节点：路由层 = AI 应用的调度器* 按任务类型/风险等级/数据敏感度/成本预算/延迟要求/评测反馈选择模型。决定什么时候便宜、什么时候可靠、什么时候拒绝执行。
...
> *知识节点：Benchmark = 路由 + 定价 + 声誉 + 结算的输入* 不只看排行榜分数，还要看稳定性/执行约定/延迟失败率/可复现性/是否刷分。好的节点获得更高流量，差的节点不进高风险场景。
...
> *知识节点：结算 = 状态机 + 规则 + 证据* AI 任务结算比普通 API 复杂（质量不是二元的）。需要：报价/参数/输出 hash/状态机（成功失败超时争议）/付款托管退款仲裁规则/日志评测证据。
...
> *知识节点：判断去中心化 AI 四问* ① 去中心化的是哪一层？② 链上负责什么？③ 相比中心化具体改善了什么？④ 开放网络的质量波动和治理成本是否值得？
...
1. 写清楚支持哪一类任务，比如合约摘要、治理提案总结或链上交易解释。
2. 网络里有哪些角色：请求方、推理节点、评测者、支付方、仲裁者。
3. 请求里包含哪些字段：模型、输入、预算、超时、隐私等级、输出格式。
4. 节点如何报价，如何返回结果，如何证明自己完成任务。
5. 结果如何验收，失败如何退款，争议如何处理。
6. 哪些信息上链，哪些信息只保存哈希或日志。
