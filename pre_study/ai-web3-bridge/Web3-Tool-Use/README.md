...
> *知识节点：Web3 工具 ≠ 普通 API 调用* 普通 API 调用出错最多返回错误码；Web3 工具出错可能改变资产所有权或授权状态。因此工具设计必须从"能用"升级到"安全地用"——读写分离、权限分层、日志不可省。
...
> *知识节点：确定性边界限制概率模型* 模型是概率系统，工具是确定性执行层。安全的设计是让工具用硬规则（白名单、额度上限、方法限制）包裹模型的自由度，而不是信任模型"不会犯错"。
...
> *知识节点：读写必须拆工具* 把"读余额"和"发交易"放在同一个 RPC 工具里 = 给 Agent 一把既能看又能花的钥匙。读写分离是 Web3 工具安全的第一道防线。
...
> *知识节点：只读 ≠ 没风险* Contract Read 不改链上状态，但可能返回过期或错误的数据。Agent 基于错误数据生成的建议仍然危险——比如用旧价格估算 swap 导致用户亏损。
...
> *知识节点：写工具 = 受限接口* 合约写入不是"调一个函数"，而是一整套检查链：网络 → 合约 → 方法 → 参数 → 额度 → 模拟 → policy → 确认。每一步都是确定性拦截点，缺任何一步都可能产生不可逆后果。
...
> *知识节点：钱包是权限边界* 钱包工具不是"签名按钮"，而是 Agent 和用户资产之间的最后一道门。必须拆分动作（连接 / 签名 / 发送 / 授权 / 撤销），让用户每次都能看清楚自己在批准什么。
...
> *知识节点：Explorer 是审计入口* Explorer Tool 让 Agent 的回答从"我觉得交易成功了"变成"根据 Etherscan 记录，这笔交易在 block #xxx 确认"。可验证性是链上 Agent 的信任基础。
...
> *知识节点：DeFi 工具 = 高危工具* DeFi 操作直接动钱。工具设计必须用白名单 + 额度 + 滑点 + simulation + policy 五层包裹，任何一层缺失都可能导致资金损失。"通用 DeFi 工具"是安全反模式。
...
> *知识节点：权限分层是安全架构* 不是所有工具调用都同权。按风险分级（只读 → 草稿 → 小额执行 → 大额确认 → 默认禁止）是 Agent 安全的基本范式。Session Key 是实现中间层自动化的关键机制。
...
> *知识节点：日志 = 事后审计链* 出问题时，日志要能还原完整链路：模型输入 → 工具调用 → 返回结果 → policy 判断 → 用户确认。没有日志，Agent 的错误就变成了"黑箱事故"，无法复盘也无法改进。
...
> *知识节点：风险随工具能力递增* 只读工具 → 解释风险；写入工具 → 资产风险；签名工具 → 权限风险。工具能力越大，安全层（policy / simulation / human check / log）必须越厚。
