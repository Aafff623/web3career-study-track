...
> *知识节点：从"相信模型"到"验证证据"* 模型输出可能触发付款 / 清算 / 授权。不能只说"模型说的"，必须有证据链：输入 → 模型版本 → 执行环境 → 输出 → 验证方式。按风险分层选择验证强度。
...
> *知识节点：验证三层* 来源验证（输入 / 模型版本 / 服务方 / 时间）→ 过程验证（可信环境 / 可重放 / 可证明）→ 影响验证（改变什么链上状态 / 是否需要挑战期）。验证成本必须匹配输出影响力。
...
> *知识节点：TEE = 硬件隔离 + attestation* 工程上相对可用，成本低于 ZK。适合"不能公开输入但需要证明运行环境"的场景。但信任假设包含硬件 + 供应链 + 远程证明服务，不是纯密码学信任。
...
> *知识节点：ZK = 密码学强验证，但成本高* 适合边界清楚、计算规模可控的任务。大型 LLM 完整推理证明目前不现实——现实路径是证明关键部分（后处理规则 / 小模型分类），不是证明整个模型大脑。
...
> *知识节点：zkML = ML 推理的 ZK 证明* 适合小型 / 结构固定 / 需要强验证的模型。设计前先问四个问题：是否需要隐藏输入？是否需要链上验证？能否接受延迟？模型能否转电路？很多场景用 TEE / 签名日志更经济。
...
> *知识节点：先明确证明对象* "证明输入没变" ≠ "证明模型版本" ≠ "证明运行环境" ≠ "证明完整计算正确"。不同目标选不同技术，不要追求"最强证明"——签名日志可能够用，ZK 最强但最贵。
...
> *知识节点：链下执行 + 链上验证* 复杂计算放链下，摘要 / 证明 / 签名放链上。但要注意数据可用性——链上只有 hash / proof 时，原始数据在哪里、谁能访问、保存多久必须清楚。
...
> *知识节点：公开 benchmark ≠ 任务级验证* 通用榜单好 ≠ 链上任务好。项目必须建自己的 benchmark：正常 + 边界 + 攻击样本（错误链 / 恶意上下文 / 过期价格 / 假文档）。
...
> *知识节点：审计轨迹是最容易落地的可验证层* 不需要 TEE / ZK，只需要完整记录：输入 → 输出 → 模型版本 → 工具调用 → 时间 → 用户确认 → tx hash。不能证明模型正确，但能证明"系统当时看到了什么"。
...
> *知识节点：混合验证策略* 日志（日常复盘）+ attestation（服务证明）+ ZK/TEE（高风险场景）+ challenge（争议处理）。不需要所有输出都用最强证明，按风险分层选择验证强度。
