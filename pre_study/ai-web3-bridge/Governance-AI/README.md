...
> *知识节点：治理 AI = 提高信息质量，不是替代政治判断* AI 帮助整理提案/会议/预算/贡献，但不能制造偏见、遗漏来源、过度简化争议。核心是减少信息不对称，不是替人投票。
...
> *知识节点：治理 AI 三原则* ① 来源可追溯（摘要能回到提案/论坛/会议/链上记录）② 立场分开（事实 ≠ 推断 ≠ 观点 ≠ 建议）③ 人保留决策权（AI 辅助，投票仍由人完成）。
...
> *知识节点：提案摘要要有固定结构 + 标注不确定性* 固定结构：目标/背景/预算/执行主体/时间线/支持理由/反对理由/未决问题。区分事实（"请求 50,000 USDC"）和判断（"预算是否合理"）。
...
> *知识节点：Meeting Action = 模糊讨论 → 可检查事项* "我们应该研究一下" ≠ action。"Alice 在下周五前整理三种支付方案" = action。要记录决策状态（已决定/待确认/需要投票/需要复核）。
...
> *知识节点：贡献图发现隐形贡献* 不是把贡献简化成 commits 数量。要发现：谁长期 review / 谁协调会议 / 谁维护文档 / 谁回答新人。帮助避免重复拨款和漏拨款。
...
> *知识节点：Budget Check = 让社区看清问题，不是自动砍预算* 检查：预算项是否完整 / 里程碑是否具体 / 过去拨款是否完成 / 金额是否匹配 / 是否重复申请。链上还要检查地址权限和历史异常。
...
> *知识节点：来源可追溯是治理 AI 底线* 每条摘要附来源（论坛链接/提案编号/会议时间戳/链上交易）。无来源摘要不应作为投票依据。无法给出来源时标注"推断"或"不确定"。
...
> *知识节点：Deep Funding = AI 构建证据包，人类做价值判断* 影响不是线性的（底层库/协调者/文档很难用 KPI 衡量）。AI 辅助构建证据包，但最终分配保留多元评审 + 公开理由 + 申诉机制。
...
> *知识节点：Plurality = 展开多元视角，不是压成平均意见* AI 摘要要展示：少数派担忧 / 利益相关者差异 / 地区角色差异 / 可协商空间。费用提案中 LP/交易者/开发者/金库管理者/token holder 视角不同，要展开而非抹平。
...
> *知识节点：治理比交易更重视正当性* AI 辅助工具必须透明 / 可质疑 / 可复核。不能悄悄替用户投票或隐藏反对意见。
