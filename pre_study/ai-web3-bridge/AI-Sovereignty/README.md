...
> *知识节点：AI 主权 = 退出 + 迁移 + 选择 + 验证* 数据能否导出、模型能否切换、工具权限能否撤销、Agent 身份能否跨平台保留。核心是用户保留退出、迁移、选择和验证的能力。
...
> *知识节点：主权三要素* ① 控制权可见（用户知道哪些数据/权限/记忆被使用）② 选择权真实（模型/钱包/工具/存储不被单点锁死）③ 可验证性保底（关键行为有日志/签名/证明/链上记录）。
...
> *知识节点：User Control = 看得见 + 改得了* 用户要能看见 Agent 用哪个模型、保存哪些记忆、拿哪些权限、能花多少钱，并能修改和撤销。控制面板至少要显示：钱包 / session key / 授权额度 / 工具 / 记忆开关 / 导出入口 / 紧急停止。
...
> *知识节点：数据可迁移 = 分层导出 + 机器可读* 不是打包下载原始数据，而是分层导出（任务历史 / 偏好 / 日志 / 凭证 / 声誉 / 私有记忆），标明敏感等级，提供 JSON schema / VC / 签名日志等机器可读格式。
...
> *知识节点：Model Choice = 策略化选择，不是手动切换* 系统允许按策略选模型：隐私优先 / 成本优先 / 质量优先 / 开源优先 / 可验证优先。高风险 Agent 不能热切换模型后继续自动执行，需要评估流程。
...
> *知识节点：Local-first = 混合架构，不是完全离线* 本地做过滤/脱敏/风险初筛 → 云端强模型只接收必要摘要 → 最终交易仍由钱包/智能账户确认。默认减少不必要外发。
...
> *知识节点：抗审查 = 开源 + fallback + 可迁移* 开源客户端、多模型 fallback、可自托管 endpoint、可迁移身份、去中心化存储、链上审计记录。关键公共基础设施避免单点控制。
...
> *知识节点：d/acc = 更强自动化 + 更好防御工具* 不只做"更强的自动代理"，还要做"更好的防御工具"：更安全的钱包 / 抗钓鱼界面 / 可验证 Agent 日志 / 隐私保护数据协作 / 开源模型评测 / 去中心化身份和声誉。


#### CROPS

难度：高级。 CROPS 概括 Ethereum 应守住的核心属性：Censorship resistance、Open source、Privacy、Security。
...
CROPS 和 d/acc 是相通的：它不是反对 AI，而是要求 AI 进入钱包、治理、身份和支付层时，不能牺牲用户控制、隐私和安全。

> *知识节点：CROPS = 产品价值检查表* Censorship resistance（不依赖单一平台）+ Open source（关键组件可检查）+ Privacy（数据最小化和隔离）+ Security（权限/工具/日志有防线）。AI 进入钱包/治理/支付层时不能牺牲这些属性。
...
> *知识节点：主权是价值底层* 没有主权设计，AI×Web3 容易变成"用链上资产喂给更中心化的 AI 平台"。Agent Wallet 控制权限 + Verifiable AI 可证明 + AI Privacy 有边界 + Agent Identity 跨平台。
...
- Ethereum Account Abstraction：理解可编程账户如何改善用户控制。
- W3C DID Core：了解可解析、可控制的去中心化身份。
- Oasis Documentation：了解隐私和可验证计算基础设施。
- ERC-8004：Agent 身份和声誉的开放标准草案。
- Ethereum Foundation Mandate：理解 CROPS 如何进入 Ethereum Foundation 的正式使命表述。
- Vitalik: My techno-optimism：理解 d/acc 的思想背景。
- Vitalik: What I would love to see in a wallet：理解 AI、钱包、隐私、安全和用户控制之间的关系。
