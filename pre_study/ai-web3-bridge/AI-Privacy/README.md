...
> *知识节点：隐私问题的本质* 不是单点泄漏，而是链上公开身份 + AI 私有上下文被拼接。地址关联 + 交易历史 + 持好 + Agent 偏好 + 聊天内容组合 = 极其具体的用户画像。
...
> *知识节点：隐私三原则* ① 数据边界显式（哪些进模型 / 哪些只进工具 / 哪些只在本地）② 记忆可管理（可查看 / 可删除 / 可导出 / 可关闭）③ 链上身份谨慎关联（不默认合并多地址与真实身份）。
...
> *知识节点：数据边界 = 数据流图，不是隐私长文* 每个字段问五个问题：是否必要 / 是否可脱敏 / 是否可本地处理 / 是否进日志 / 是否给第三方。Agent 工具间数据不应默认互通。
...
> *知识节点：Local AI 原则 = 先本地筛选脱敏，再发摘要给强模型* 不要上传全部历史，先本地提取"与任务有关的 3 笔交易"。但本地 ≠ 绝对安全，仍需 sandbox + 权限控制。
...
> *知识节点：Private Memory 不能是黑箱* 用户要知道记了什么 / 为什么记 / 如何删除 / 是否用于训练 / 是否跨设备同步。分层管理（短期偏好 / 敏感身份 / 钱包记录），默认最小化、可查看、可删除。
...
> *知识节点：Secret Management 底线* secret 不进 prompt / 不进模型输出 / 不进日志 / 不进 analytics。通过受控工具间接使用，不直接读取。助记词和私钥即使用户粘贴也要拒绝处理。
...
> *知识节点：最小披露 = 摘要代替原始数据* 证明"余额足够"不暴露全部持仓，证明"有权限"不公开真实身份。技术手段：ZK 证明 / 一次性地址 / 按应用隔离身份 / summary 替代原文。
...
> *知识节点：加密 ≠ 模型使用隐私* 服务器加密保存但推理时解密发给模型 = 存储安全，不是使用隐私。加密要和 TEE / 访问控制 / 最小化上下文结合。关键是：谁持有密钥。
...
> *知识节点：Consent 要具体到动作和数据类型* "同意连接钱包" ≠ "同意分析全部历史"。每个操作是独立开关。Consent 可撤销，但要说明：哪些能删、哪些链上记录无法删、哪些日志因合规保留。
...
> *知识节点：组合风险是核心* 单个地址 = 公开信息。但地址 + 聊天记录 + 邮箱 + 地理位置 + 社群身份 = 真实身份画像。Agent 越懂用户，泄漏后伤害越大。
